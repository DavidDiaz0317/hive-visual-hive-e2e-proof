...
1. The standard `npm test` command runs Vitest with `--passWithNoTests`, but there is deliberately no JavaScript unit-test file. The Python component tests are real project tests, while Visual Hive layer 2 should still emit one `test_adequacy_gap` recommending focused repository unit evidence. This lets Hive repair the gap by adding only a safe test file; the existing runner then executes that test in required checks without a package or workflow change.
